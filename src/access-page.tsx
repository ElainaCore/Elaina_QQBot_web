...
function isInjected(bot: ApiData) {
  return bot.connection_type === "QQ 连接" || bot.runtime_mode === "QQ 连接";
}
...
export function AccessCenterPage() {
  ...
  const injectProcess = async (process: ApiData) => {
    ...
    try {
      ...
      toast(`QQ 进程 PID ${process.pid} 已连接`);
      await load();
    } catch (error) {
      ...
      <Dialog open={addOpen} onOpenChange={setAddOpen}>
        <DialogContent>
          <div className="pr-8">
            ...
            <DialogDescription className="mt-1">{addStep === "choose" ? "选择 QQ 运行方式或接入已有 OneBot 实现。" : windows ? "选择一个已经登录的 QQ 主进程并连接账号。" : "创建后启动 QQ，并通过扫码或快捷登录接入。"}</DialogDescription>
          </div>
          {addStep === "choose" ? (
            <div className="mt-6 space-y-3"><AccessMethod icon={windows ? MonitorSmartphone : Bot} title={windows ? "连接 QQ" : "内置 QQ"} detail={windows ? "选择本机已登录的 QQ 进程并连接账号" : "由框架启动 Linux QQ，通过扫码登录并保存账号会话"} onClick={() => setAddStep("qq")} /><AccessMethod icon={Network} title="OneBot 接入" detail="使用正向或反向 WebSocket、HTTP 接入已有实现端" onClick={chooseOneBot} /></div>
          ) : windows ? (
            <div className="mt-6 space-y-3"><div className="flex items-center justify-between"><p className="text-xs text-muted-foreground">检测到 {externalProcesses.length} 个 QQ 主进程</p><Button size="sm" variant="outline" onClick={() => void load()}><RefreshCw className="size-3.5" />刷新</Button></div>{externalProcesses.length ? externalProcesses.map((process) => <div key={process.id || process.pid} className="flex flex-col gap-3 rounded-lg border border-border/70 p-4 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="text-sm font-medium">{process.name || process.process_name || "QQ"}</span><Badge variant={process.status === "online" ? "success" : process.status === "loaded" ? "warning" : "secondary"}>{process.status === "online" ? "已连接" : process.status === "loaded" ? "已准备" : "可连接"}</Badge></div><p className="mt-1 text-xs text-muted-foreground">PID {process.pid}{process.uin ? ` · QQ ${process.uin}` : ""}{process.memory_rss_mb ? ` · ${process.memory_rss_mb} MB` : ""}</p>{process.path && <p className="mt-1 truncate text-xs text-muted-foreground" title={String(process.path)}>{process.path}</p>}</div><Button size="sm" disabled={!process.can_load || process.status === "online" || busy === `inject:${process.pid}`} onClick={() => void injectProcess(process)}>{busy === `inject:${process.pid}` ? <Loader2 className="size-3.5 animate-spin" /> : <MonitorSmartphone className="size-3.5" />}{process.status === "online" ? "已连接" : "连接"}</Button></div>) : <div className="rounded-lg border border-dashed border-border p-10 text-center"><MonitorSmartphone className="mx-auto size-8 text-muted-foreground/35" /><p className="mt-3 text-sm text-muted-foreground">未检测到 QQ 主进程，请先安装、启动并登录 QQ。</p></div>}<Button variant="ghost" size="sm" onClick={() => setAddStep("choose")}>返回接入方式</Button></div>
          ) : (
