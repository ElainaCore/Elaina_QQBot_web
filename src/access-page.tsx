...
function isEmbedded(bot: ApiData) {
  return Boolean(bot.bot_id);
}

function isInjected(bot: ApiData) {
  return bot.connection_type === "注入 QQ" || bot.runtime_mode === "注入 QQ";
}
...
export function AccessCenterPage() {
  ...
  const detachInjected = async (bot: ApiData) => {
    if (!bot.pid || !await dialog.confirm({ title: "解除接管", description: `解除 QQ ${bot.qq || bot.bot_qq} 的接管？`, confirmLabel: "解除", destructive: true })) return;
    ...
    <section className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex shrink-0 gap-2">
          ...
          <Button size="sm" onClick={openAdd} disabled={loading || !platform}><Plus className="size-3.5" />添加接入</Button>
        </div>
      </div>
      ...
      {view === "onebot" ? <ConnectionsPage compact createSignal={connectionSignal} /> : (
        <>
                  ...
                  <Card key={botId || bot.bot_qq || index} className="overflow-hidden">
                    <CardContent className="p-5 pt-6 sm:p-6">
                      ...
                      <div className="mt-4 flex flex-wrap gap-2">
                        ...
                        {injected && <Button size="sm" variant="outline" disabled={busy === `detach:${bot.pid}`} onClick={() => void detachInjected(bot)}><Unplug className="size-3.5" />解除接管</Button>}
                        {!embedded && !injected && <Button size="sm" variant="outline" onClick={() => setView("onebot")}><Link2 className="size-3.5" />管理接入</Button>}
                      </div>
                    </CardContent>
                  </Card>
                );
          ...
          ) : <div className="rounded-lg border border-dashed border-border p-12 text-center"><Bot className="mx-auto size-9 text-muted-foreground/35" /><h3 className="mt-4 text-sm font-medium">还没有 QQ 账号</h3><p className="mt-1 text-xs text-muted-foreground">从右上角添加 QQ 或 OneBot 接入</p><Button className="mt-4" size="sm" onClick={openAdd}><Plus className="size-3.5" />添加接入</Button></div>}

          {!loading && (windows ? (
            <Card><CardHeader className="flex-row flex-wrap items-start justify-between gap-3"><div className="min-w-0"><CardTitle>Windows QQ</CardTitle><CardDescription className="break-all">{qqStatus.qq_executable || "安装并登录系统 QQ 后，可从添加接入中选择进程注入"}</CardDescription></div><Badge className="shrink-0" variant={qqStatus.installed ? "success" : "secondary"}>{qqStatus.installed ? "已检测到" : "未检测到"}</Badge></CardHeader><CardContent><Button variant="outline" onClick={() => window.open(String(qqStatus.official_download_url || "https://im.qq.com/index/#/"), "_blank", "noopener,noreferrer")}><ExternalLink className="size-3.5" />前往 QQ 官网</Button></CardContent></Card>
          ) : (
      ...
      <Dialog open={addOpen} onOpenChange={setAddOpen}>
        <DialogContent>
          <div className="pr-8">
            <DialogTitle>{addStep === "choose" ? "选择接入方式" : windows ? "选择 QQ 进程" : "添加内置 QQ"}</DialogTitle>
            <DialogDescription className="mt-1">{addStep === "choose" ? "选择 QQ 运行方式或接入已有 OneBot 实现。" : windows ? "选择一个已经登录的 QQ 主进程进行注入和接管。" : "创建后启动 QQ，并通过扫码或快捷登录接入。"}</DialogDescription>
          </div>
          {addStep === "choose" ? (
            <div className="mt-6 space-y-3"><AccessMethod icon={windows ? MonitorSmartphone : Bot} title={windows ? "注入 QQ" : "内置 QQ"} detail={windows ? "选择本机已登录的 QQ 进程，注入运行时并接管账号" : "由框架启动 Linux QQ，通过扫码登录并保存账号会话"} onClick={() => setAddStep("qq")} /><AccessMethod icon={Network} title="OneBot 接入" detail="使用正向或反向 WebSocket、HTTP 接入已有实现端" onClick={chooseOneBot} /></div>
          ) : windows ? (
            <div className="mt-6 space-y-3"><div className="flex items-center justify-between"><p className="text-xs text-muted-foreground">检测到 {externalProcesses.length} 个 QQ 主进程</p><Button size="sm" variant="outline" onClick={() => void load()}><RefreshCw className="size-3.5" />刷新</Button></div>{externalProcesses.length ? externalProcesses.map((process) => <div key={process.id || process.pid} className="flex flex-col gap-3 rounded-lg border border-border/70 p-4 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="text-sm font-medium">{process.name || process.process_name || "QQ"}</span><Badge variant={process.status === "online" ? "success" : process.status === "loaded" ? "warning" : "secondary"}>{process.status === "online" ? "已接管" : process.status === "loaded" ? "已注入" : "可注入"}</Badge></div><p className="mt-1 text-xs text-muted-foreground">PID {process.pid}{process.uin ? ` · QQ ${process.uin}` : ""}{process.memory_rss_mb ? ` · ${process.memory_rss_mb} MB` : ""}</p>{process.path && <p className="mt-1 truncate text-xs text-muted-foreground" title={String(process.path)}>{process.path}</p>}</div><Button size="sm" disabled={!process.can_load || process.status === "online" || busy === `inject:${process.pid}`} onClick={() => void injectProcess(process)}>{busy === `inject:${process.pid}` ? <Loader2 className="size-3.5 animate-spin" /> : <MonitorSmartphone className="size-3.5" />}{process.status === "online" ? "已接管" : "注入并接管"}</Button></div>) : <div className="rounded-lg border border-dashed border-border p-10 text-center"><MonitorSmartphone className="mx-auto size-8 text-muted-foreground/35" /><p className="mt-3 text-sm text-muted-foreground">未检测到 QQ 主进程，请先安装、启动并登录 QQ。</p></div>}<Button variant="ghost" size="sm" onClick={() => setAddStep("choose")}>返回接入方式</Button></div>
          ) : (
